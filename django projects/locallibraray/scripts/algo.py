...
from BotTrade.models import Portfolio, Daylog, PortfolioDb, OrderHistoryDb
import pytz
...
def initPortfolio():
	...
	tempPort=PortfolioDb.objects.filter(userObject=user_Object)	
	
	for i in tempPort.values():
		userPortfolio.append(i)
		
	for i in tempPort:
		portfolio[i.stockName]=i.quantity
...
def find20BBBounds(symbol):
	df = web.DataReader(symbol, data_source = 'yahoo', start = datetime.now() - timedelta(days = 40), end = datetime.now())
...
def decide(symbol):
	...
	Daylog.objects.create(symbol=symbol,time=datetime.now(tz_NY),price=price,upperBand=upperBand,lowerBand=lowerBand,bandWidth=bandWidth,rsi=rsi,upperSlope=upperSlope,realSlope=realSlope, lowerSlope=lowerSlope, upperI=upperI, realI=realI, lowerI=lowerI, action=action)
	# # fObj.write(line)
	# fObj.close()
	return action